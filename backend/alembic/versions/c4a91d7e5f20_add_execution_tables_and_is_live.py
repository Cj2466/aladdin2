...
def upgrade() -> None:
    ...
    # a NOT NULL column without one. sa.false() (not sa.text('0')) is required
    # here, not cosmetic either: Postgres does not implicitly cast an integer
    # literal to boolean in a DEFAULT clause and rejects `DEFAULT 0` outright,
    # whereas sa.false() renders dialect-correctly on both SQLite and
    # Postgres -- matching the established, already-Postgres-proven pattern
    # this codebase already uses for every other boolean column default (see
    # is_verified/auto_backtests_triggered in earlier migrations).
    op.add_column(
        'strategy_portfolios',

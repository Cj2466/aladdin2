...
def test_create_and_get_portfolio(client, register_and_verify):
...
def test_list_portfolios_only_returns_own(client, register_and_verify):
    register_and_verify(client, email="owner@example.com")
    client.post("/api/portfolios", json=_sample_portfolio_payload("Owner's portfolio"))

    other_client = TestClient(app)
    register_and_verify(other_client, email="other@example.com")
...
def test_get_portfolio_not_owned_is_404(client, register_and_verify):
    register_and_verify(client, email="owner2@example.com")
    create_response = client.post("/api/portfolios", json=_sample_portfolio_payload())
    portfolio_id = create_response.json()["id"]

    other_client = TestClient(app)
    register_and_verify(other_client, email="intruder@example.com")
    response = other_client.get(f"/api/portfolios/{portfolio_id}")
    assert response.status_code == 404


def test_get_nonexistent_portfolio_is_404(client, register_and_verify):
    register_and_verify(client)
    response = client.get("/api/portfolios/99999")
    assert response.status_code == 404


def test_update_portfolio_replaces_holdings(client, register_and_verify):
    register_and_verify(client)
    create_response = client.post("/api/portfolios", json=_sample_portfolio_payload())
    portfolio_id = create_response.json()["id"]
...
def test_delete_portfolio(client, register_and_verify):
    register_and_verify(client)
    create_response = client.post("/api/portfolios", json=_sample_portfolio_payload())
    portfolio_id = create_response.json()["id"]
...
def test_analyze_saved_portfolio_caches_second_call(client, register_and_verify, canned_prices, monkeypatch):
    fetch_calls = []
...
def test_delete_portfolio_cascades_risk_results(
    client, register_and_verify, canned_prices, monkeypatch, test_db_engine
):
    ...
    register_and_verify(client, email="cascade_check@example.com")
    create_response = client.post("/api/portfolios", json=_sample_portfolio_payload())
    portfolio_id = create_response.json()["id"]
...
def test_analyze_portfolio_not_owned_is_404(client, register_and_verify, canned_prices, monkeypatch):
    def fake_get_price_history(tickers, start, end):
        present = [t for t in tickers if t in canned_prices.columns]
        missing = [t for t in tickers if t not in canned_prices.columns]
        return canned_prices[present], missing
    ...
    register_and_verify(client, email="owner3@example.com")
    create_response = client.post("/api/portfolios", json=_sample_portfolio_payload())
    portfolio_id = create_response.json()["id"]

    other_client = TestClient(app)
    register_and_verify(other_client, email="intruder2@example.com")

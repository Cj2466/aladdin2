...
def main() -> int:
    ...
    for model in ("normal", "t4", "ar1"):
        ...
        print(f"  (1-ALPHA) quantile of max_k PSR under {model:<6} = {c_by_model[model]:.5f}")
    worst = max(c_by_model, key=c_by_model.get)
    c_k = c_by_model[worst]
    print(f"  constant per-look boundary C_K = {c_k:.5f}  (calibrated under the worst model: {worst})")
    ...
    results = {}
    for label, model, crit in (("G1 normal", "normal", ALPHA), ("G2a t4", "t4", 0.07), ("G2b ar1 phi=0.3", "ar1", 0.07)):
    ...
    print(f"G4: with {N_FAMILIES} null Dormant families, expected false promotions over {K_MAX} years "
          f"= {N_FAMILIES * float((promotion_look(psr_path(gen(grng, 'normal', 0.0, 4000)), c_k) > 0).mean()):.1f} "
          f"(each = a forward-tracking slot, not capital)")

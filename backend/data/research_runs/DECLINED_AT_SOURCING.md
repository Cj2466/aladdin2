...
### 18. `us_odd_lot_self_tender` — measured on a full EDGAR census (2026-09-12, Opus measurement, orchestrator recounted the population, the premium distribution and one primary-document hash exactly)

| # | family key | source and claim | rule that fails | verdict |
|---|---|---|---|---|
| 18 | `us_odd_lot_self_tender` | Retail-only by rule: 17 CFR 240.13e-4(f)(3)(i) permits an issuer tender to accept ALL shares of holders of "less than one hundred shares" without proration (permissive: 257 offers grant, 7 deny). R1 PASS on primary evidence — split-off exchange offers filled odd-lot holders in full while everyone else was prorated to 6.99% (Cummins), 7.24% (Danaher), 14.82% (McKesson) etc., quoted and hashed in `measurements_m2_2026-09-12/`. R2 PASS by construction. R6 PASS. Census 2019-01 → 2026-09: 1,175 "odd lot" SC TO-I filings → 348 offers / 176 issuers → **103 real bets (listed, grants priority, common stock) from 79 issuers**; per year 17/17/19/16/15/8/5/6 (2019→2026). Gross premium vs the document's own pre-announcement price: median **+4.21%** (p25 +0.91%, p75 +10.71%, N 78), 82% positive, 95% completed, median holding 29 days; median 99-share stake $2,382 → median gross gain **$63**. | **R3: 5–19 bets/yr at ~$63 each is not a member of anything; 14 of 78 lose after a $2 round trip; the bet count fell 15 → 5 over 2023–2025.** Kadapakkam-Zhang-Yildirim 2021 (profits "disappeared") is not contradicted — the +4.21% is a different quantity (offer vs pre-announcement, not tendering profit). | DECLINE_AT_SOURCING on R3, now on its OWN measured bet count and dollar size |

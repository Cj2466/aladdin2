...
| # | item | status |
|---|---|---|
| 2c.1 | Set 2: 212 Open Source Asset Pricing predictors, one pre-declared look (`postpub_lockbox_2026-09-12/`) | **VERDICT REVISED 2026-09-15 — PASS → FAIL AS BUILT.** The arithmetic is correct and was reproduced exactly by an independent re-derivation (195 members, 612 months, mean 0.4202 %/mo, SR 0.5991), but the 20/P bps haircut rests on `DEFAULT_XS_COST_BPS = 5.0`, which its own definition site calls "not independently recalibrated". A2 then priced the same series at a SOURCED curve (SEC/Collver 2014, MIDAS intraday data): at the micro-cap rate one reformation costs 184–247 bp against a 30.6 bp break-even → **Sharpe −1.53 to −0.95**. OSAP equal-weights 210 of 242 predictors on CRSP, so micro-cap is the right row. A liquid-only variant is UNTESTED, not passing — its gross edge cannot be recovered from OSAP. See `a2_cost_2026-09-15/RESULT_A2_2026-09-15.md` addendum and `number_audit_2026-09-15/` |
...
## 2c-bis. Number, data and tool audit (2026-09-15, owner-requested) — READ BEFORE TRUSTING ANY EARLIER NUMBER

Owner: "เช็คทุกอย่างให้ถูกต้องที่สุดก่อน". Orchestrator did it directly, not delegated.
Reports: `number_audit_2026-09-15/NUMBER_AUDIT_2026-09-15.md` (numbers) and
`DATA_AND_TOOL_AUDIT_2026-09-15.md` (panel + estimator).

| id | item | status |
|---|---|---|
| 2c-bis.1 | Nine checks verified CLEAN: OSAP source hashes unchanged; headline reproduces exactly; NW(6) genuinely applied (4.2693); √12 annualization is CONSERVATIVE (Lo 2002 → 0.672); `M_eff = avg_var/var_pool` correct; the √M identity exact (ratio 1.000) on one sample; POWER_FLOOR 0.80 and SCREENING_FLOOR 0.50 documented, not invented; 2.416 certifiability bar confirmed | done |
| 2c-bis.2 | **P1** the PASS verdict rested on an unsourced constant → superseded by A2 | closed by 2d.3 |
| 2c-bis.3 | **P2** 252 of 612 months hold <20 members (1974-01 holds two); the "51 years" framing describes a book that did not exist for two decades | must travel with every statement of the result |
| 2c-bis.4 | **P3** orchestrator's own error: the conversational "0.169 × √14.6 = 0.645 ≈ 0.597" check is in no committed file and mixed three populations; correct identity on one sample is 0.1751 × √35.04 = 1.0364 | corrected, table withdrawn |
| 2c-bis.5 | **P4** PSR ≥ 0.95 clears at any Sharpe above ~0.21 at n=612 — the pass line was effectively one condition, not two | recorded |
| 2c-bis.6 | **P5** two research branches unmerged | **CLOSED 2026-09-15, owner approved.** Both merged (`a5449b1`, `08af531`), docs only, zero app/tests files. The holding-period table was independently re-derived cell by cell first and reproduces exactly (P=1 n=98 median 0.291, P=3 n=7 0.333, P=6 n=2 0.199, P=12 n=87 0.255, P=36 n=1 0.122) |
| 2c-bis.7 | **P6/D6** no borrow cost, retail micro-cap borrow often unavailable at any price; delisting outcome unknown for 1,235 of 1,817 dead names, so the survivorship correction is partial | open |
| 2c-bis.13 | **NEW, opened by the 2026-09-15 fix**: `ipo_lockup_expiration`, `jump_drift`, `eigenportfolio` and `patterns` have STALE stored results — their cost basis changed and 26% of cells now take the flat `cost_bps` fallback, which is itself optimistic for a stock too illiquid to measure. **None of their numbers may be quoted until re-run against a same-day baseline.** The better fix is a per-liquidity-bucket sourced rate, now buildable from the SEC/Collver curve A2 found | **open** |
| 2c-bis.8 | **P8** the sealed dataset's one-look budget is drawn down; this audit's own sub-window figures (0.9568, 1.0364, M_eff 35.04) are diagnostics that may NEVER be promoted to a verdict | binding |
| 2c-bis.9 | **D1** `_coverage.json` is a request log, not a coverage ledger — 1,821 symbols (12.6%) claim bars through 2026-09-11 when their last real bar is years earlier | FIXED for A2 via `a2_universe_2026-09-15/clean_universe.csv`; the store's own file is still wrong |
| 2c-bis.10 | **D2** ~1,966 likely unflagged reverse splits (flags reconcile at 92% where present) | masked in A2; the store is unchanged |
| 2c-bis.11 | **D3** the panel is **51.0% ETPs and only 37.3% common stock**, not the 8.7% non-stock a ticker-pattern heuristic suggested | FIXED for A2 |
| 2c-bis.12 | **D5** the two uncalibrated EDGE entry points | **DONE 2026-09-15, owner approved** (`estimator_sign_fix_2026-09-15/`). Both switched to `sign=True`. **My premise to the owner was STALE and is corrected**: the 0.5946 → 0.7456 switch was already made on 2026-09-05, so NO live registration was affected — proven, not assumed (calibrated frame hash identical before/after: 9504763528740016484). Measured on a same-day baseline: charged cells 70.9% → 44.8%, median change on surviving cells +0.00 bp, and the 8,821 withdrawn cells (26.0%) had been charged a median 49.00 bp manufactured from volatility |
...
| 2d.3 | **A2** per-ticker spread/cost estimate on the whole-market panel | **DONE 2026-09-15** (`a2_cost_2026-09-15/`, `a2_universe_2026-09-15/`). Run under the nine rules the 2026-09-15 audit set. 4,355 common-stock tickers priced; per-liquidity-quintile one-way half-spread, tick floor 18.38 → 0.49 bp (37×) vs EDGE 106 → 16 bp (6.5×). EDGE validated on synthetic data with a KNOWN spread: accurate ≥25 bp, returns a volatility-proportional noise floor below that (at true spread 0 it returns +2.65/+7.27/+20.43 bp at daily vol 50/150/400 bp in the package-default mode). Level anchor taken from the SEC's own MIDAS study, not invented. **The prior guess in this row was HALF WRONG**: 5 bps is indeed too expensive for large caps, but for micro caps it is too cheap by 9–12×, not merely "far too cheap" |

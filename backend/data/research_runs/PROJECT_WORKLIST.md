...
| 2c-bis.6 | **P5** `spread-estimator-audit-2026-09-12` and `holding-period-2026-09-13` are still UNMERGED branches | **open — owner decision** |
...
| 2c-bis.12 | **D5** `estimate_effective_spread` and `build_edge_half_spread_frame` still call EDGE in the package-default (spread-manufacturing) mode, and `ipo_lockup_expiration` consumes one; the project measured the difference on lazy_prices at net Sharpe 0.5946 → 0.7456 | **open — NOT changed, rule 6: this is a live dependency and the owner's call** |

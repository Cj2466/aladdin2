...
Find a small number of genuinely real, statistically and economically defensible trading edges — never a manufactured or uncorrected positive result — in service of the project owner's longer-term ambition: a Renaissance/Medallion-style research program built on many independent, small, honestly-verified micro-edges combined at scale, rather than any single "big idea" factor. Capital preservation and honesty take priority over maximizing any individual result or hit rate. Getting the measurement right (this file's rules) is necessary but not sufficient — it makes false positives less likely, it does not manufacture edges that don't exist. The actual bottleneck is finding genuinely new, not-yet-arbitraged mechanisms to test (see project research history/memory for what's already been tried and ruled out), not further refining how already-exhausted ideas get scored.

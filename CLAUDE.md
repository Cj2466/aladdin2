...
4. **Model choice follows an architect/builder split, decided actively per task — not a rigid assignment.** Default posture: **Opus is the architect** — it plans, holds the overall picture, and is responsible for catching risks, edge cases, or blind spots a narrower implementation pass would miss. **Sonnet is the builder** — it executes work against a plan that's already been laid out. This is a default, not a hard rule: if a specific piece of the *building* itself genuinely needs Opus-level judgment (a subtle methodological call, adversarial/skeptical verification, synthesizing conflicting results, a safety-critical review), use Opus there too — evaluate honestly per task, the same active-duty judgment this rule has always required, rather than following the architect/builder split mechanically once it stops fitting the work in front of you. `claude-fable-5-1` is banned outright until the project owner explicitly lifts the ban.

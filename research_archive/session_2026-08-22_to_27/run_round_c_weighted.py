"""Round C re-run with magnitude-weighted leg sizing (was equal-weighted).
Same window, same 30-pattern family, same universe as the original run —
the ONLY change is the sizing mechanism inside cross_sectional.py."""
import sys
import time
...
REPORT_PATH = "/private/tmp/claude-501/-Users-choonhakunjaroonwatthana-Desktop-aladdin2/be436747-4a23-4713-9fbf-e7e19fc0ad51/scratchpad/round_c_weighted_report.txt"

start_time = time.time()
print(f"Starting Round C (magnitude-weighted) screening: {MEMBERSHIP_DATA_START} -> {date.today()}", flush=True)
...
lines = []
lines.append(f"Round C (magnitude-weighted legs) — completed in {elapsed:.1f}s")
...
lines.append(f"Specs that produced a usable replay: {len(results)} / 30")
lines.append("")
